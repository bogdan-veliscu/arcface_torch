...
class face_learner(object):
    def __init__(self, conf, inference=False):
        ...
        if conf.use_mobilfacenet:
            ...
            self.model = Backbone(conf.net_depth, conf.drop_ratio, conf.net_mode).to(
                conf.device
            )
        ...
        if not inference:
            ...
            self.step = 0
            self.head = Arcface(
                embedding_size=conf.embedding_size, classnum=self.class_num
            ).to(conf.device)
    ...
    def save_state(
        self, conf, accuracy, to_save_folder=False, extra=None, model_only=False
    ):
        if to_save_folder:
        ...
        if not model_only:
            torch.save(
                ...
                save_path
                / (
                    "head_{}_accuracy:{}_step:{}_{}.pth".format(
                        get_time(), accuracy, self.step, extra
                    )
                ),
            )
            torch.save(
                ...
                save_path
                / (
                    "optimizer_{}_accuracy:{}_step:{}_{}.pth".format(
                        get_time(), accuracy, self.step, extra
                    )
                ),
            )
    ...
    def board_val(self, db_name, accuracy, best_threshold, roc_curve_tensor):
        ...
        self.writer.add_scalar(
            "{}_best_threshold".format(db_name), best_threshold, self.step
        )
        self.writer.add_image(
            "{}_roc_curve".format(db_name), roc_curve_tensor, self.step
        )
        ...
        for e in range(epochs):
            ...
            for imgs, labels in tqdm(iter(self.loader)):
                ...
                if self.step % self.evaluate_every == 0 and self.step != 0:
                    ...
                    self.board_val(
                        "agedb_30", accuracy, best_threshold, roc_curve_tensor
                    )
                    accuracy, best_threshold, roc_curve_tensor = self.evaluate(
    ...
    def infer(self, conf, faces, target_embs, tta=False):
        ...
        for img in faces:
            if tta:
                ...
                embs.append(
                    self.model(conf.test_transform(img).to(conf.device).unsqueeze(0))
                )
